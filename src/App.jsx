      ...
      return (
        <div className="min-h-screen bg-white dark:bg-[#1e293b] flex items-center justify-center p-6 font-mono text-left z-50 relative">
          <div className="bg-red-900/20 border border-red-500/30 p-8 rounded-2xl max-w-4xl w-full">
            ...
            <button onClick={() => window.location.reload()} className="mt-8 px-6 py-3 bg-slate-50 dark:bg-[#0f172a] hover:bg-slate-100 dark:bg-[#1e293b] text-slate-900 dark:text-white rounded-xl font-bold transition-colors w-full sm:w-auto">Reload Application</button>
          </div>
        </div>
      );
    }
      ...
      <BrowserRouter>
        <Routes>
          <Route path="/" element={<Layout user={user} onLogout={handleLogout} vehicles={vehicles} bookings={bookings} />}>
            ...
            <Route path="settings" element={(user.role === 'admin' || user.role === 'reservation') ? <Settings companyDetails={companyDetails} setCompanyDetails={setCompanyDetails} /> : <Navigate to="/" replace />} />
