        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                <div className="space-y-1.5">
                    <h2 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Driver Fleet</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">Manage your certified tour operators and dispatch statuses.</p>
                </div>
                <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredDrivers.map((driver) => (
                    <div key={driver._id} className="glass-card hover:bg-slate-100 dark:bg-[#1e293b] border-slate-300 dark:border-white/10 transition-colors group relative overflow-visible flex flex-col justify-between">
                        <div>
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex items-center gap-3">
                                    <div className="h-12 w-12 rounded-full bg-slate-50 dark:bg-[#0f172a] border-2 border-emerald-500/20 flex items-center justify-center text-emerald-400 font-bold text-xl">
                                        {driver.name.charAt(0)}
                                    </div>
                                    <div>
                                        <h3 className="text-slate-900 dark:text-white font-bold">{driver.name}</h3>
                                ...
                                <div className="relative">
                                    ...
                                    {openDropdown === driver._id && (
                                        <div className="absolute right-0 top-10 w-40 bg-slate-50 dark:bg-[#0f172a] border border-slate-300 dark:border-white/10 shadow-2xl rounded-xl z-20 overflow-hidden animate-in fade-in zoom-in-95">
                            ...
                            <div className="mt-4 mb-2">
                                {(driver.tasks && driver.tasks.length > 0) ? (
                                    <div className="text-sm font-medium text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-[#0f172a]/50 p-3 rounded-lg border border-slate-200 dark:border-white/5">
                                    ...
                                    <div className="text-sm font-medium text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-[#0f172a]/50 p-3 rounded-lg border border-slate-200 dark:border-white/5">
                                        <span className="text-slate-500 dark:text-slate-400 text-xs block mb-1">Legacy Task:</span>
                                        {driver.task}
                                    </div>
                                ) : (
                                    <div className="text-sm font-medium text-slate-500 dark:text-slate-400 italic bg-slate-50 dark:bg-[#0f172a]/50 p-3 rounded-lg border border-slate-200 dark:border-white/5">
                                        No tasks assigned.
                                    </div>
                                )}
                            </div>
            ...
            <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Register New Driver">
                <form className="space-y-4" onSubmit={handleRegisterDriver}>
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                        <button type="button" onClick={() => setIsAddModalOpen(false)} className="px-5 py-2.5 rounded-xl font-bold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:text-white transition-colors">Cancel</button>
                ...
                <div className="space-y-6">
                    ...
                    <div className="max-h-60 overflow-y-auto space-y-3 pr-2">
                        {selectedDriver?.tasks?.length === 0 ? (
                            <p className="text-slate-500 dark:text-slate-400 text-sm text-center py-4 bg-slate-50 dark:bg-[#0f172a]/50 rounded-xl border border-slate-200 dark:border-white/5">No active or completed tasks.</p>
                        ) : (
                            selectedDriver?.tasks?.map((task, index) => (
                                <div key={index} className={`p-4 rounded-xl border ${task.status === 'completed' ? 'bg-emerald-500/5 border-emerald-500/20' : 'bg-slate-50 dark:bg-[#0f172a] border-slate-300 dark:border-white/10'} relative group`}>
                                    ...
                                    <div className="text-xs text-slate-600 dark:text-slate-300 space-y-1">
                                        ...
                                        {selectedDriver.driverType === 'safari' && task.park && (
                                            <>
                                                ...
                                                {task.comment && (
                                                    <div className="mt-1 text-slate-600 dark:text-slate-300 italic border-l-2 border-primary-500/30 pl-2">
                                                        "{task.comment}"
                                                    </div>
                                                )}
                                            </>
                    ...
                    {/* Add/Edit Task Form */}
                    <form onSubmit={handleSaveTask} className="space-y-4 bg-slate-50 dark:bg-[#0f172a]/50 p-4 rounded-xl border border-slate-200 dark:border-white/5">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input name="date" required type="date" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].date : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Time</label>
                                <input name="time" required type="time" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].time : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Pickup Location</label>
                                <input name="pickupLocation" required type="text" placeholder="e.g. JKIA" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].pickupLocation : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Dropoff Location</label>
                                <input name="dropoffLocation" required type="text" placeholder="e.g. Hotel" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].dropoffLocation : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                        </div>

                        <div className="grid grid-cols-3 gap-4">
                            <div className="col-span-1">
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Plane size={10} /> Flight No. (Opt)</label>
                                <input name="flightNumber" type="text" placeholder="e.g. KQ100" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].flightNumber : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                            <div className="col-span-1">
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Clock size={10} /> Flight Arr. (Opt)</label>
                                <input name="flightArrivalTime" type="time" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].flightArrivalTime : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                            <div className="col-span-1">
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Clock size={10} /> SGR Arr. (Opt)</label>
                                <input name="sgrArrivalTime" type="time" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].sgrArrivalTime : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Users size={10} /> Guests</label>
                                <input name="guests" required type="number" min="1" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].guests : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                            <div>
                                <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 flex items-center gap-1"><Car size={10} /> Vehicle Type</label>
                                <input name="vehicleType" required type="text" placeholder="e.g. Land Cruiser" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].vehicleType : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                            </div>
                        </div>
                        ...
                        {selectedDriver?.driverType === 'safari' && (
                            <div className="bg-primary-500/5 border border-primary-500/20 p-3 rounded-xl mt-4 space-y-4">
                                ...
                                <div>
                                    <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">National Park / Reserve</label>
                                    <select name="park" required defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].park : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Days</label>
                                        <input name="days" required type="number" min="1" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].days : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Nights</label>
                                        <input name="nights" required type="number" min="0" defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].nights : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500" />
                                    </div>
                                </div>
                                <div>
                                    <label className="block text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Admin/Reservation Comment (Optional)</label>
                                    <textarea name="comment" rows="2" placeholder="Add notes for this week/two-weeks safari..." defaultValue={editingTaskIndex !== null ? selectedDriver.tasks[editingTaskIndex].comment : ''} className="w-full bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-primary-500 resize-none"></textarea>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="pt-2 flex justify-end gap-2">
                            {editingTaskIndex !== null && (
                                <button type="button" onClick={() => { setEditingTaskIndex(null); }} className="px-4 py-2 text-sm rounded-lg font-bold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:text-white transition-colors border border-slate-300 dark:border-white/10">Discard Edit</button>
                            )}
