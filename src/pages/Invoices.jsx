    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-4xl sm:text-5xl font-black text-slate-900 dark:text-white tracking-tight mb-3">Customer Invoices</h1>
                    <p className="text-slate-500 dark:text-slate-400 text-lg">Generate formal PDF receipts for confirmed Safari bookings.</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div className="glass-card p-6 flex flex-col gap-2">
                    <div className="text-slate-500 dark:text-slate-400 text-sm font-bold uppercase tracking-wider">Total Generated</div>
                    <div className="text-3xl font-black text-slate-900 dark:text-white">{invoiceableBookings.length}</div>
                </div>
                <div className="glass-card p-6 flex flex-col gap-2 border-emerald-500/20 bg-emerald-500/5">
                    ...
                    <div className="text-3xl font-black text-slate-900 dark:text-white">{invoiceableBookings.filter(b => b.paymentStatus === 'paid').length}</div>
                </div>
                <div className="glass-card p-6 flex flex-col gap-2 border-orange-500/20 bg-orange-500/5">
                    ...
                    <div className="text-3xl font-black text-slate-900 dark:text-white">{invoiceableBookings.filter(b => b.paymentStatus === 'pending').length}</div>
                </div>
            </div>
                                ...
                                {invoiceableBookings.map((booking) => (
                                    <tr key={booking._id} className="hover:bg-slate-100 dark:bg-[#1e293b] transition-colors group">
                                        ...
                                        <td className="p-4 whitespace-nowrap">
                                            <div className="text-slate-900 dark:text-white font-medium">{typeof booking.price === 'number' ? booking.price.toLocaleString() : booking.payment}</div>
                                        </td>
                                        <td className="p-4 whitespace-nowrap">
