...
const ProgressStat = ({ label, current, total, color }) => {
    ...
    return (
        <div className="space-y-2">
            <div className="flex justify-between text-xs font-medium">
                ...
                <span className="text-slate-900 dark:text-white">{current}/{total}</span>
            </div>
...
const FleetOverview = ({ vehicles, drivers }) => {
    const vAvailable = vehicles?.filter(v => v.status === 'available').length || 0;
    const vAssigned = vehicles?.filter(v => v.status === 'assigned').length || 0;
    const vMaintenance = vehicles?.filter(v => v.status === 'maintenance').length || 0;

    const dAvailable = drivers?.filter(d => d.status === 'available').length || 0;
    const dAssigned = drivers?.filter(d => d.status === 'assigned').length || 0;
    const dLeave = drivers?.filter(d => d.status === 'on-leave').length || 0;

    return (
        <div className="glass-card mt-8">
            <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-6">Fleet & Resource Status</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                <div className="space-y-6">
                    <h4 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Vehicles</h4>
                    <ProgressStat label="Available" current={vAvailable} total={vehicles?.length} color="emerald" />
                    <ProgressStat label="On Mission" current={vAssigned} total={vehicles?.length} color="blue" />
                    <ProgressStat label="Maintenance" current={vMaintenance} total={vehicles?.length} color="orange" />
                </div>
                <div className="space-y-6">
                    <h4 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Drivers</h4>
                    <ProgressStat label="Active" current={dAvailable} total={drivers?.length} color="emerald" />
                    <ProgressStat label="On Mission" current={dAssigned} total={drivers?.length} color="blue" />
                    <ProgressStat label="On Leave" current={dLeave} total={drivers?.length} color="red" />
                </div>
            </div>
        </div>
    );
...
const Dashboard = ({ bookings, drivers, vehicles, user }) => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
            ...
            <FleetOverview vehicles={vehicles} drivers={drivers} />
            ...
            {/* Admin Chart Section */}
            {user?.role === 'admin' && (
                <div className="glass-card mt-8 p-6 border border-slate-200 dark:border-white/5">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <select className="bg-white dark:bg-[#1e293b] border border-slate-300 dark:border-white/10 text-slate-900 dark:text-white text-sm rounded-lg px-3 py-2 outline-none focus:border-primary-500/50">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">
                <div className="lg:col-span-2 glass-card">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white">Recent Deployments</h3>
                        <button className="text-sm text-primary-400 hover:text-primary-300 transition-colors">View All</button>
                    </div>

                    <div className="space-y-4">
                        {bookings?.slice(0, 5).map((booking) => (
                            <div key={booking._id} className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-[#0f172a]/50 border border-slate-200 dark:border-white/5 hover:bg-slate-100 dark:bg-[#1e293b] transition-colors">
                                <div className="flex items-center gap-4">
                                    <div className={`p-2 rounded-lg ${booking.status === 'confirmed' ? 'bg-emerald-500/10 text-emerald-400' :
                                        booking.status === 'pending' ? 'bg-orange-500/10 text-orange-400' :
                                            'bg-dark-500/10 text-slate-500 dark:text-slate-400'
                                        }`}>
                                    ...
                                    <div>
                                        <h4 className="text-slate-900 dark:text-white font-medium">{booking.clientName}</h4>
                                        <p className="text-xs text-slate-500 dark:text-slate-400">{booking.destination} &bull; {booking.startDate}</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    {user?.role === 'admin' && <p className="text-slate-900 dark:text-white font-bold">{booking.payment.toLocaleString()}</p>}
                ...
                <div className="glass-card flex flex-col gap-6">
                    <div>
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">AI Insurance Alerts</h3>
                        <div className="space-y-3">
                            ...
                            {aiAlerts.filter(a => a.id.startsWith('ins-')).length === 0 && (
                                <p className="text-slate-500 dark:text-slate-400 text-sm italic">AI Tracker: All policies active.</p>
                            )}
                        </div>
                    </div>
                </div>
