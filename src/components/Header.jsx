    ...
    return (
        <header className="sticky top-0 z-30 flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8 bg-white/80 dark:bg-white/10 backdrop-blur-lg border-b border-slate-200 dark:border-white/5">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{getPageTitle()}</h1>
                    <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 hidden sm:block">Welcome back, {user?.name || 'User'}</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-5">
                ...
                <div className="relative" ref={notifRef}>
                    ...
                    {showNotifications && (
                        <div className="absolute right-0 mt-2 w-80 sm:w-[26rem] glass-card bg-white dark:bg-[#1e293b] !p-0 border-slate-300 dark:border-white/10 shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 z-50">
                            <div className="p-4 border-b border-slate-200 dark:border-white/5 bg-slate-100 dark:bg-[#1e293b] flex justify-between items-center">
                            ...
                            <div className="max-h-96 overflow-y-auto">
                                {alerts.length === 0 ? (
                                    ...
                                    alerts.map(alert => (
                                        <div key={alert.id} className="p-4 border-b border-slate-200 dark:border-white/5 hover:bg-slate-100 dark:bg-[#1e293b] transition-colors group flex justify-between items-start">
                ...
                <div className="flex items-center gap-3 cursor-pointer hover:bg-slate-100 dark:bg-[#1e293b] p-1 pr-3 rounded-full transition-colors border border-transparent hover:border-slate-300 dark:border-white/10">
                    <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-primary-600 to-orange-500 flex items-center justify-center shadow-lg shadow-orange-500/20">
                        <span className="text-slate-900 dark:text-white font-bold text-sm tracking-widest">{user?.name?.charAt(0) || 'U'}</span>
                    </div>
