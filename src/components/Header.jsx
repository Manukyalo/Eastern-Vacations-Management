...
const Header = ({ toggleSidebar, user, vehicles, bookings }) => {
    ...
    return (
        <header className="sticky top-0 z-30 flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8 bg-white/80 backdrop-blur-lg border-b border-slate-200">
            <div className="flex items-center gap-4">
                <button
                    onClick={toggleSidebar}
                    className="p-2 -ml-2 rounded-xl text-slate-600 hover:text-slate-900 hover:bg-slate-200 lg:hidden transition-colors"
                >
                    <Menu size={24} />
                </button>
                <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">{getPageTitle()}</h1>
                    <p className="text-xs sm:text-sm text-slate-500 hidden sm:block">Welcome back, {user?.name || 'User'}</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-5">
                ...
                <div className="relative" ref={notifRef}>
                    <button
                        ...
                        className={`relative p-2 rounded-xl transition-colors ${showNotifications ? 'bg-slate-200 text-slate-900' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200'}`}
                    >
                    ...
                    {showNotifications && (
                        <div className="absolute right-0 mt-2 w-80 sm:w-[26rem] glass-card bg-white !p-0 border-slate-300 shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2 z-50">
                            ...
                            <div className="max-h-96 overflow-y-auto">
                                {alerts.length === 0 ? (
                                    ...
                                    alerts.map(alert => (
                                        <div key={alert.id} className="p-4 border-b border-slate-200 hover:bg-slate-100 transition-colors group flex justify-between items-start">
                                            ...
                                            <button
                                                ...
                                                className="p-1.5 text-slate-500 opacity-0 group-hover:opacity-100 peer hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-all"
                ...
                <div className="flex items-center gap-3 cursor-pointer hover:bg-slate-100 p-1 pr-3 rounded-full transition-colors border border-transparent hover:border-slate-300">
                    <div className="h-9 w-9 rounded-full bg-gradient-to-tr from-primary-600 to-orange-500 flex items-center justify-center shadow-lg shadow-orange-500/20">
                        <span className="text-slate-900 font-bold text-sm tracking-widest">{user?.name?.charAt(0) || 'U'}</span>
                    </div>
